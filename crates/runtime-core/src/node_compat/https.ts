...
function buildUrl(input: string | URL | RequestOptionsLike): string {
  ...
  if (!host) {
    throw new TypeError("https.request requires hostname/host or absolute URL");
  }
...
function createServer(): never {
  return notImplemented("https.createServer");
}
...
function get(
  ...
  const options = typeof optionsOrCb === "object" && optionsOrCb !== null ? optionsOrCb : undefined;
  const cb = (typeof optionsOrCb === "function" ? optionsOrCb : maybeCb) ?? undefined;
  const req = request(input, { ...(options ?? {}), method: "GET", protocol: "https:" }, cb);
  req.end();
  return req;
}

const httpsModule = { Agent, IncomingMessage, ClientRequest, createServer, request, get };

export { Agent, IncomingMessage, ClientRequest, createServer, request, get };
